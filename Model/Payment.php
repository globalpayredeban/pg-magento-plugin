...
namespace GlobalPay\Module\Model;
...
use Payment\Payment as GPPayment;

class Payment extends Cc
{
    const METHOD_CODE = 'globalpay_module';
    ...
    )
    {
        ...
        $this->_logger->info("Init GlobalPay Constructor");
    ...
    public function capture(InfoInterface $payment, $amount)
    {
        ...
        } catch (PaymentErrorException $paymentError) {
            $this->_logger->error("Error in capture => " . $paymentError->getMessage());
            throw new MagentoValidatorException(__('Payment capturing error. Detail: ' . $paymentError->getMessage()));
        }
    ...
    public function authorize(InfoInterface $payment, $amount)
    {
        ...
        try {
            ...
            $this->_logger->error("Error in $service => " . $paymentError->getMessage());
            throw new MagentoValidatorException(__("Payment $service error. Detail: " . $paymentError->getMessage()));
        }
    ...
    public function refund(InfoInterface $payment, $amount)
    {
        ...
        try {
            ...
            $this->_logger->error("Error in refund => " . $paymentError->getMessage());
            throw new MagentoValidatorException(__('Payment refund error. Detail: ' . $paymentError->getMessage()));
        }
    ...
    private function initPaymentSDK()
    {
        $isProduction = boolval((integer)$this->getConfigData('staging_mode')) ? false : true;

        // Initalize Payment SDK
        $credentials = $this->getServerCredentials();

        if (empty($credentials)) {
            throw new MagentoValidatorException(__('[GLOBALPAY]: Missing client credentials.'));
        }

        GPPayment::init($credentials['code'], $credentials['api_key'], $isProduction);

        $this->_service = GPPayment::charge();
    }
